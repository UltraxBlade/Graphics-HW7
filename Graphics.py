...
def drawPolys(img,edges,color):
    ...
    for col in range(0,len(edges[0])-1,3):
        ...
        if dotProduct(norm,view)>0:
            line(img,int(edges[0][col]),int(edges[1][col]),int(edges[0][col+1]),int(edges[1][col+1]),color[:])
            line(img,int(edges[0][col+1]),int(edges[1][col+1]),int(edges[0][col+2]),int(edges[1][col+2]),color[:])
            line(img,int(edges[0][col]),int(edges[1][col]),int(edges[0][col+2]),int(edges[1][col+2]),color[:])
