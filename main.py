...
def oldParse(filename,edges,polys,transform,img,color):
    f=open(filename,"r")
    script=f.read().split("\n")
    f.close()
    i=0
...
def newParse(filename,world,img,color):
    f=open(filename,"r")
    script=f.read().split("\n")
    f.close()
    i=0
    while i<len(script):
        ...
        if script[i]=="line":
            ...
            oldParse(script[i+1],edges,polys,I(4),img,drawColor)
            i+=1
        if len(edges[0])>0:
            edges=multMatrix(transform,edges)
            drawLines(img,edges,color)
...
if not newMode:
    ...
    polys=[[],[],[],[]]
    transform=I(4)
    oldParse("waluigiTriangles.txt",edges,polys,transform,img,drawColor)
else:
    world=WorldStack()
    newParse("script.txt",world,img,drawColor)
